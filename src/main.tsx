...
function AdminApp() {
  ...
  return <main className="shell admin"><header><div className="eyebrow">PENGELOLA LAYANAN</div><h1>Siapkan pelanggan.</h1><p>Atur layanan dari satu tempat. Pelanggan cukup membuka layanan yang sudah siap.</p></header><div className="admin-layout"><aside><div className="section-title"><span>BUYER</span><button onClick={() => { const name = prompt("Nama pelanggan"); const telegramId = prompt("ID Telegram pelanggan"); if (name && telegramId) void api("/api/admin/buyers", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ name, telegramId }) }).then(load); }}>+ tambah</button></div>{overview.buyers.map((item) => <button className={`buyer-row ${selected === item.id ? "chosen" : ""}`} key={item.id} onClick={() => setSelected(item.id)}><b>{item.name}</b><span>{item.planBroadcast || item.planComment ? "siap" : "baru"}</span></button>)}</aside><section className="setup"><div className="section-title"><span>PENGATURAN {buyer?.name}</span><b>{buyer?.broadcastActive || buyer?.commentActive ? "aktif" : "belum aktif"}</b></div><div className="access-pills"><label><input type="checkbox" checked={form.planBroadcast} onChange={(e) => update("planBroadcast", e.target.checked)} /> Auto Sebar</label><label><input type="checkbox" checked={form.planComment} onChange={(e) => update("planComment", e.target.checked)} /> Auto Komen</label></div>{form.planBroadcast && <fieldset><legend>Auto Sebar · akun khusus</legend><label>Akun kerja<select value={form.workerId} onChange={(e) => update("workerId", e.target.value)}><option value="">Pilih akun</option>{overview.workers.filter((item) => !item.buyerId || item.buyerId === selected).map((item) => <option key={item.id} value={item.id}>{item.label} · @{item.username} · {item.status}</option>)}</select></label><button className="small-button" onClick={() => void addWorker()}>+ Tambah akun</button><label>Wording<textarea value={form.wording} onChange={(e) => update("wording", e.target.value)} placeholder="Wording promosi buyer…" /></label><label>15 grup publik, satu per baris<textarea value={form.groups} onChange={(e) => update("groups", e.target.value)} placeholder="@grup_satu\n@grup_dua" /></label><label>Jeda akun (menit)<input value={form.intervalMinutes} inputMode="numeric" onChange={(e) => update("intervalMinutes", e.target.value)} /></label></fieldset>}{form.planComment && <fieldset><legend>Auto Komen · akun buyer</legend><label>Base tujuan<textarea value={form.bases} onChange={(e) => update("bases", e.target.value)} placeholder="@base_satu\n@base_dua" /></label><label>Divisi<input value={form.division} onChange={(e) => update("division", e.target.value)} /></label><label>Keyword yang dicari<textarea value={form.keywords} onChange={(e) => update("keywords", e.target.value)} placeholder="netflix\nnetflix premium" /></label><label>Blacklist keyword OOT<textarea value={form.blacklist} onChange={(e) => update("blacklist", e.target.value)} placeholder="jasa edit\nbarter" /></label><label>Wording komentar<textarea value={form.commentWording} onChange={(e) => update("commentWording", e.target.value)} /></label><label>Metode<select value={form.mode} onChange={(e) => update("mode", e.target.value)}><option value="APPROVAL">By approval</option><option value="AUTO">Otomatis</option></select></label></fieldset>}<button className="save" onClick={() => void submit()}>Simpan & siapkan buyer</button>{message && <div className="notice">{message}</div>}</section></div></main>;
}
